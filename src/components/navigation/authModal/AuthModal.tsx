...
const AuthModalContent = ({
  ...
  const [isOpen, setIsOpen] = useState(!!isAfterLogout || isDefaultOpen);
  ...
  const onEmailSubmit = async () => {
    setIsLoading(true);
    try {
      const res = await fetcher(`${apiUrl}/private/users/login`, {
        method: "POST",
  ...
  const onCodeSubmit = async (codeValue) => {
    setIsLoading(true);
    try {
      const res = await fetcher(`${apiUrl}/private/users/verify`, {
        method: "POST",
  ...
  return (
    <Dialog
      open={isOpen}
    >
      {!isDefaultOpen && <DialogTrigger asChild={type === "onboarding"}>
        {type === "login" ? (
          "Log in"
        ) : (
          <Button
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription className="text-center text-2xl text-black uppercase font-bold px-6">
